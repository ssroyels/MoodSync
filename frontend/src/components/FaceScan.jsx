...
const Navbar = ({ username }) => (
  <div className="bg-gradient-to-r from-purple-900 to-indigo-700 text-white py-4 px-6 shadow-xl flex justify-between items-center">
    <h1 className="text-2xl font-bold tracking-wide animate-pulse">🎵 MoodSync</h1>
...
const FaceScan = () => {
    ...
    const detectExpression = () => {
      intervalRef.current = setInterval(async () => {
        ...
        if (result?.expressions) {
          const maxExp = Object.entries(result.expressions).reduce((a, b) => (a[1] > b[1] ? a : b));
  ...
  const saveExpressionToDB = async (mood1) => {
    try {
      await axios.post("https://mood-sync-one.vercel.app/mood/save", {
        userId: user?.email,
        mood: mood1,
      });
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-800 text-white">
      ...
      <div className="text-center mt-6 px-4">
        <h2 className="text-3xl font-bold mb-4 animate-fade-in">🎭 Face Expression Detection</h2>
        <video
          ref={videoRef}
          autoPlay
          muted
        ...
        <p className="mt-6 text-lg">
          Detected Expression: {" "}
          <span className="text-yellow-400 font-semibold text-xl">{expression}</span>
        </p>
